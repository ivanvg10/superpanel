    ...
    <div className="grid grid-cols-3 gap-3 mb-8">
      {stats.map(({ label, value, unit }) => (
        <div key={label} className="bg-ios-elev border border-ios-sep rounded-ios p-4 shadow-card">
          ...
          <p className="font-display font-bold text-2xl text-ios-label leading-none">
            {value}
          </p>
  ...
  return (
    <motion.div {...fadeUp} className="min-h-full">
      ...
      <div className="border-b border-ios-sep px-5 pt-7 pb-4 flex items-center justify-between sticky top-0 bg-ios-bg/80 backdrop-blur-xl z-10">
        <div>
          <h2 className="text-[28px] font-bold tracking-tight text-ios-label">Gym</h2>
              ...
              <motion.section key={month} {...staggerItem}>
                ...
                <div className="space-y-2">
                  {items.map((s) => (
                    ...
                    >
                      <div className="w-8 h-8 bg-ios-blue/10 border border-ios-blue/20 rounded-ios flex items-center justify-center flex-shrink-0">
                        <Dumbbell className="w-4 h-4 text-ios-blue" />
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        {s.notes && (
                          <p className="text-xs text-ios-label3 mt-1 line-clamp-1">{s.notes}</p>
                        )}
                      </div>
                      <button
